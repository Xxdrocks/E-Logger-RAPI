...
function ProtectedRoute({ children, adminOnly = false, superadminOnly = false }) {
    ...
    if ((adminOnly || superadminOnly) && !user) {
        return <Navigate to="/login" replace />;
    }

    if (superadminOnly && user?.role !== 'superadmin') {
        return <AccessDenied message="Halaman ini hanya untuk superadmin" />;
    }

    if (adminOnly && !['admin', 'superadmin'].includes(user?.role)) {
        return <AccessDenied message="Halaman ini hanya untuk admin atau superadmin" />;
    }
