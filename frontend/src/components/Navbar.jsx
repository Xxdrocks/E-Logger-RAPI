...
function Navbar() {
    ...
    return (
        <div>
            ...
            <nav className="md:hidden fixed bottom-6 w-87 left-1/2 -translate-x-1/2 z-50">
                <div className="bg-white/30 backdrop-blur-xl border border-white/40 rounded-3xl shadow-2xl shadow-indigo-200/50 p-2">
                        ...
                        {navItems.map((item) => (
                            ...
                            >
                                <img
                                    src={item.image}
                                    alt={item.name}
                                    className={`w-6 h-auto object-contain transition-all duration-300 ${active === item.path ? 'scale-110' : 'opacity-70'
                                        }`}
                                 />
                            </Link>
                        ))}
